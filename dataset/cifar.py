...
from .util import ImageFolder, TinyImageFolder, ImageFolder_fix

logger = logging.getLogger(__name__)

__all__ = ['cifar10_mean', 'cifar10_std', 'cifar100_mean', 'cifar100_std', 'normal_mean',
           'normal_std', 'TransformFixMatch', 'TransformFixMatch_Imagenet',
           'TransformFixMatch_Imagenet_Weak', 'TransformFixMatch_TinyImagenet_Weak']
...
def get_cifar(args, norm=True):
    ...
    train_labeled_idxs, train_unlabeled_idxs, val_idxs = \
        ...
        #split_l_u(args, base_dataset.targets, tot_class, num_unlabeled)
...
def get_tinyimagenet(args, norm=True):
    mean = normal_mean
    std = normal_std
    all_classes = 200
    txt_file = "data/tiny-imagenet/train_all.txt"
    labeled_per_class = args.num_labeled
    id_class_number = args.num_super

    args.num_classes = id_class_number

    unlabeled_number = 40000
    unlabeled_ratio = args.ratio

    unlabel_number_per_id_class = int(unlabeled_number * (1.0 - unlabeled_ratio)) // id_class_number
    n_unlabels_shift = (unlabeled_number - (unlabel_number_per_id_class * id_class_number)) // \
                       (all_classes - id_class_number)
    ...
    for i in range(id_class_number):
    ...
    for i in range(id_class_number, all_classes):
    ...
    val_txt_file = "data/tiny-imagenet/val_all.txt"

    with open(val_txt_file, 'r') as f:
        annos = [[line.split(' ')[0], int(line.split(' ')[1])] for line in f.readlines()]

    val_idxs += [anno[0] for anno in annos]
    val_labs += [anno[1] for anno in annos]

    ## This function will be overwritten in trainer.py
    norm_func = TransformFixMatch_TinyImagenet(mean=mean, std=std,
                                           norm=norm, size_image=32)

    dataset_labeled = TinyImageFolder(labeled_idxs, labeled_labs, transform=norm_func)
    dataset_unlabeled = TinyImageFolder(unlabeled_idxs, unlabeled_labs, transform=norm_func)
    ...
    dataset_val = TinyImageFolder(val_idxs, val_labs, transform=test_transform)
    dataset_test = TinyImageFolder(val_idxs, val_labs, transform=test_transform)
...
def split_l_u(args, labels, all_classes, num_unlabeled):
    ...
    unlabel_number_per_id_class = int(num_unlabeled * (1.0 - args.ratio)) // args.num_classes
    '''
    unlabel_number_per_ood_class = (
       num_unlabeled - (unlabel_number_per_id_class * args.num_classes)
    ...
    for i in range(args.num_classes):
        ...
        unlabeled_idx.extend(idx[
             label_per_class+val_per_class:label_per_class+val_per_class+unlabel_number_per_id_class
        ])

    for i in range(args.num_classes, all_classes):
        idx = np.where(labels == i)[0]
        unlabeled_idx.extend(
            idx[:n_unlabels_shift])
...
class TransformFixMatch_TinyImagenet_Weak(object):
    def __init__(self, mean, std, norm=True, size_image=32):
        self.weak = transforms.Compose([
            transforms.Scale((32, 32)),
            transforms.RandomHorizontalFlip(),
